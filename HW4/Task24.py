...
kol = int(input('Сколько кустов на грядке?: '))
gr = range(kol)
arr = []
for i in gr:
    n = int(input(f'Введите количество ягод на кусте {i+1}:'))
    arr.append(n)

berrys = []
for i in range(len(arr) - 1):
    berrys.append(arr[i-1] + arr[i] + arr[i+1])
berrys.append(arr[-2] + arr[-1] + arr[0])
print(max(berrys))
